...
import com.socks.jiandan.okhttp.parser.CommentListParser;
import com.socks.jiandan.okhttp.parser.FreshNewsCommentParser;
...
//        }), mActivity);
        ...
        OkHttpProxy.get(Commentator.getUrlCommentList(thread_key), mActivity, new OkHttpCallback<ArrayList<Commentator>>(new CommentListParser(new LoadFinishCallBack() {
            @Override
            public void loadFinish(Object obj) {
                thread_id = (String) obj;
            }
            ...
            @Override
            public void onSuccess(int code, ArrayList<Commentator> response) {
                if (response.size() == 0) {
                    mLoadResultCallBack.onSuccess(LoadResultCallBack.SUCCESS_NONE, null);
                } else {
                    commentators.clear();
...
//        }), mActivity);
        ...
        OkHttpProxy.get(Comment4FreshNews.getUrlComments(thread_key), mActivity, new OkHttpCallback<ArrayList<Comment4FreshNews>>(new FreshNewsCommentParser(new LoadFinishCallBack() {
            @Override
            public void loadFinish(Object obj) {
                thread_id = (String) obj;
            }
            ...
            @Override
            public void onSuccess(int code, ArrayList<Comment4FreshNews> response) {
                if (response.size() == 0) {
                    mLoadResultCallBack.onSuccess(LoadResultCallBack.SUCCESS_NONE, null);
                } else {
                    commentators4FreshNews.clear();
